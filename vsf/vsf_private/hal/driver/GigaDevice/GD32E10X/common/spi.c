...
static const IRQn_Type spi_irq_list[SPI_COUNT][2] = {   // DMA RX, SPI
    #if SPI0_ENABLE
    {DMA0_Channel1_IRQn, SPI0_IRQn},
    #endif  // SPI0_ENABLE
    #if SPI1_ENABLE
    {DMA0_Channel3_IRQn, SPI1_IRQn},
    #endif  // SPI1_ENABLE
    #if SPI2_ENABLE
    {DMA1_Channel0_IRQn, SPI2_IRQn},
    #endif  // SPI2_ENABLE
};
...
void vsfhal_spi_init(enum spi_idx_t idx)
{
    ...
    case SPI0_IDX:
        ...
        RCU_APB2RST &= ~RCU_APB2RST_SPI0RST;
        AFIO_PCF0 = (AFIO_PCF0 & ~AFIO_PCF0_SPI0_REMAP) | (SPI0_IO_REMAP << 0);
    ...
    case SPI2_IDX:
        ...
        RCU_APB1RST |= RCU_APB1RST_SPI2RST;
        RCU_APB1RST &= ~RCU_APB1RST_SPI2RST;
        AFIO_PCF0 = (AFIO_PCF0 & ~AFIO_PCF0_SPI2_REMAP) | (SPI2_IO_REMAP << 28);
        #if SPI2_NSS_IO_IDX != GPIO_INVALID_IDX
        vsfhal_gpio_init(SPI2_NSS_IO_IDX);
        vsfhal_gpio_config(SPI2_NSS_IO_IDX, 0x1 << SPI2_NSS_IO_PIN, IO_OUTPUT_AFPP);
        #endif
        #if SPI2_SCK_IO_IDX != GPIO_INVALID_IDX
        vsfhal_gpio_init(SPI2_SCK_IO_IDX);
        vsfhal_gpio_config(SPI2_SCK_IO_IDX, 0x1 << SPI2_SCK_IO_PIN, IO_OUTPUT_AFPP);
        #endif
        #if SPI2_MISO_IO_IDX != GPIO_INVALID_IDX
        vsfhal_gpio_init(SPI2_MISO_IO_IDX);
        vsfhal_gpio_config(SPI2_MISO_IO_IDX, 0x1 << SPI2_MISO_IO_PIN, IO_INPUT_FLOAT);
        #endif
        #if SPI2_MOSI_IO_IDX != GPIO_INVALID_IDX
        vsfhal_gpio_init(SPI2_MOSI_IO_IDX);
        vsfhal_gpio_config(SPI2_MOSI_IO_IDX, 0x1 << SPI2_MOSI_IO_PIN, IO_OUTPUT_AFPP);
        #endif
        break;
    #endif
    }
...
void vsfhal_spi_config(enum spi_idx_t idx, uint32_t kHz, uint32_t config)
{
    ...
    if (dmax) {
        spi_control[idx].dma_idx = (dmax - DMA0) / (DMA1 - DMA0);
...
#endif  // SPI1_ENABLE
#if SPI2_ENABLE
ROOT void SPI2_IRQHandler(void) {spi_irq(SPI2_IDX);}
#endif  // SPI2_ENABLE
