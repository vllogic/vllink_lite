...
vsf_err_t mt006_usbh_init(mt006_usb_t *hc, usb_hc_ip_cfg_t *cfg)
{
    ...
    __mt006_usb_init_interrupt(hc, cfg->priority);
    return VSF_ERR_NONE;
}

void mt006_usbh_get_info(mt006_usb_t *hc, usb_hc_ip_info_t *info)
{
    VSF_ASSERT(info != NULL);
    info->regbase = hc->param->reg;
    info->ep_num = mt006_usbh_ep_number;
    info->is_dma = mt006_usbh_ep_is_dma;
}

void mt006_usbh_irq(mt006_usb_t *hc)
{
    __mt006_usb_irq(hc);
}
