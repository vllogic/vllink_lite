...
static const mt006_usb_const_t __USB_OTG##__N##_const = {                       \
    USB_OTG##__N##_CONFIG                                                       \
};                                                                              \
mt006_usb_t USB_OTG##__N##_IP = {                                               \
    .param = &__USB_OTG##__N##_const,                                           \
};                                                                              \
ROOT void USB_OTG##__N##_IRQHandler(void)                                       \
{                                                                               \
    if (USB_OTG##__N##_IP.is_host) {                                            \
        mt006_usbh_irq(&USB_OTG##__N##_IP);                                     \
    } else {                                                                    \
        mt006_usbd_irq(&USB_OTG##__N##_IP);                                     \
    }                                                                           \
}
...
WEAK(mt006_usbd_irq)
void mt006_usbd_irq(mt006_usb_t *dc) { VSF_HAL_ASSERT(false); }
WEAK(mt006_usbh_irq)
void mt006_usbh_irq(mt006_usb_t *dc) { VSF_HAL_ASSERT(false); }
