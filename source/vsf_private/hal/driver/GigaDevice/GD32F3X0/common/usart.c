...
static const IRQn_Type usart_irq_list[USART_COUNT][2] = {   // DMA, TX RX
    #if USART0_ENABLE
    {DMA_Channel1_2_IRQn, USART0_IRQn},
    #endif  // USART0_ENABLE
    #if USART1_ENABLE
    {DMA_Channel3_4_IRQn, USART1_IRQn},
    #endif  // USART1_ENABLE
};
...
void vsfhal_usart_config(enum usart_idx_t idx, uint32_t baudrate, uint32_t mode)
{
    ...
    if (dmax) {
        usart_control[idx].dma_idx = dmax;
